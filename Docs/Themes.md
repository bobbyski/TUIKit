...
| `border` | `borderForeground`, `borderBackground`, `borderStyle` (**window frames** — double means top level, i.e. windows whose parent is the desktop; nested panels draw `borderStyle.inner`, which maps `double` → `single`; dialogs are single too unless a detached one opts into `.double`) · `dividerStyle` (**interior** lines — dividers, split bars, separators; usually `single` even when frames are `double`) · `dividerConnection` (`welded` — tee/cross junctions where lines meet, Borland-style — or `notWelded` — plain unattached lines; gates welding *together with* each `Divider`'s own `isConnected`) |
