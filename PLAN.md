...
Phase 19 · Text Metrics Adoption      ████████████████████████░░   92%  🔄 v1 2026-09-09 — `DisplayWidth` was written and half-adopted; wave two converted 51 more sites (menu bars, tabs, pop-ups, completion lists, toolbars, forms, gauges, navigators, dialogs and all five chart axes) from character counts to columns. Covered by `DisplayWidthAdoptionTests` (17 tests, each confirmed failing when its fix is reverted). Remaining: `TextField`'s scroll model mixes character indices with a column budget and needs converting rather than patching
