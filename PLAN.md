...
Phase 10 · VTG Vector Graphics        ████████████████████████░░   92%  🔄 Core shipped (10.1–10.5 first pass, 10.7; Ambiance theme). Wider control pass done 2026-09-09 — `ProgressIndicator`, `Slider` and `LevelIndicator` joined, the three whose cell form rounds a number to the nearest column: a sub-cell fill, a sub-cell handle, and rounded segments, each with its cell twin shipping unchanged beside it (ground rule 8). 20 of 80 controls emit chrome, which is every control where vectors say something cells cannot. **10.8 is the only item left and cannot be closed from here**: how the chrome looks on a real VectorTerminal needs a person in front of one. Everything a headless test can check is checked
Phase 11 · Controls v3                ██████████████████████████  100%  ✅ v1 2026-09-09 — 11.1/11.3/11.4 shipped inside Phase 16; 11.5 tooltips (`TUIView.toolTip`, hover dwell, a panel that never takes focus) and 11.2 sheets (`Sheet`, anchored under its host's title row, blocking only that window) landed together
...
Phase 16 · Control Parity             ██████████████████████████  100%  ✅ Complete 2026-08-21 — Waves A+B in-package, Wave C siblings TUITerminal (no nested VTG), TUIDiagram, TUIBoards; plus 16.29 PreferencesDialog and the inline presentation (2.5), and 16.30 HexView (2026-09-09)
...
Phase 19 · Text Metrics Adoption      ██████████████████████████  100%  ✅ v1 2026-09-09 — `DisplayWidth` was written and half-adopted; wave two converted 51 more sites (menu bars, tabs, pop-ups, completion lists, toolbars, forms, gauges, navigators, dialogs and all five chart axes) from character counts to columns, and `TextField`'s scroll model became column-based rather than mixing a character index with a column budget. Covered by `DisplayWidthAdoptionTests` (20 tests, each confirmed failing when its fix is reverted)
Phase 20 · Frame Cost                 ██████████████████████████  100%  ✅ v1 2026-09-09 — `present()` already diffed rows; compositing did not. `renderTree` culls a subtree whose clip has no area, the VTG owner id is built only where chrome is live, `TUIKIT_VTG_LOG` is read once instead of per view per frame, and `effectiveTheme` is cached behind a global epoch bumped by `setNeedsDisplay`/`invalidateThemeDependentLayout` — structural, so a seventh cascade input cannot be added without invalidating. A 200-row stack in a 60-row window: **17.3ms → 3.5ms a frame, 5.0×**. Also `TableView.RowSource` (pull rows instead of holding them) and `TextField.onBeginEditing`/`onEndEditing`
```
...
| 2.5 | Inline presentation | ✅ Done (2026-08-21) | `ANSIDriver(presentation: .inline(rows:))`: no alternate screen — the app takes the next N rows at the cursor (CPR-located; scrolls just enough when near the bottom), mouse rows are remapped, frames are offset (`ANSIEncoder.frame(originRow:)`), and on exit the cursor is parked on the line BELOW the last drawn row so the shell continues under the prompt, which stays on screen. Cells only (no VTG). For "a utility that needs a couple of parameters" (Bobby). `swift run TUIKitInlineDemo`; pure parts test-pinned, driver path proven on a pty harness. |
...
| 11.2 | Sheets | ✅ Done 2026-09-09 | `Sheet: Dialog`, built with the window it belongs to. `App.presentSheet` anchors it under the host's title row, centred and clamped to the desktop, and re-anchors on resize so the thing it hangs from cannot slide out from under it. **It is deliberately not `isModal`**: that flag makes the key window swallow every click aimed anywhere else, which is what a sheet exists to avoid. The host is blocked instead, by the window router handing its presses to the sheet — so every other window in the stack stays live. Six tests, one driving a real run loop to prove the host is blocked and a sibling window is not. |
...
| 16.19 | `MasterDetail` | ✅ Done (2026-08-21; `MasterDetail` + `SidebarList`, tiles or pushes by width. Renamed from `Sidebar` per Bobby: the TUIKit sidebar IS `SlideOut`. Gallery: its own Data page, full width so the split form shows; the window's Controls slide-out is the sidebar proper, with `SidebarList` inside it) | One-call assembly over `SplitView` + `ListView`/`TreeView` + detail; `adaptive` below a width threshold pushes the detail on a `Navigator` instead of tiling. Rows gain icon + title + subtitle (`SidebarList` row style). |
| 16.20 | `ImageView` + `ImageViewer` | ✅ Done (2026-08-21; card + menu in cells, pixels under VTG) | **Placeholder + menu.** Cells: framed card `🖼 logo.png · 640×480 · PNG` with context menu Open in Viewer / Copy / Paste (paste replaces the image from the pasteboard path or data). VTG: `chrome.image` fitted/filled/centred, tint ignored. 2026-08-22: raster rides the overlay plane on terminals whose under-text pass cannot draw it (`GraphicsCapabilities.supportsUnderTextRaster`, read from the capability reply's `underText` list — VectorTerminal's Metal build is one) and the fit math keeps a 2:1 cell aspect (was a 4:1 squash). `ImageViewer` = `FloatingWindow` hosting an `ImageView` with zoom/pan (VTG) or the card (cells). Absorbs 11.3. |
...
| 16.29 | `PreferencesDialog` | ✅ Done (2026-08-21, added per Bobby: a dialog presenting preference pages — a `Toolbox` strip across the top (`.toolbar`) or a `SidebarList` beside them (`.split`). The gallery uses it for real — Settings button / File ▸ Preferences… (^P) edit a `Preferences` suite (theme, panel-at-launch, hints) the gallery boots from — and the Buttons tab opens that same dialog in each variation, toolbar and split) | `DocumentController`: open/save/save-as via `FileDialog`, dirty tracking, recents, window title `•` marker, close-confirm `Dialog`. OmegaCLIDE and the browser both reinvent this today. |
| 16.30 | `HexView` | ✅ Done (2026-09-09, added on an ActiveUI finding: `AUIHexView` was classed a *drawing* because macOS paints it with `CGContext`, and its terminal arm composed rows into a `Label` — which reads correctly and cannot carry a caret) | Address, hex-pair and text columns; the caret is inverted in the active column and underlined in the other; arrows, PageUp/PageDown, Home/End, Tab across the columns, two hex digits to commit a byte, click to place, wheel to scroll. A fitted row rounds down to a whole `bytesPerGroup` so the address column steps by 0x10 rather than 0x11. The text column is a `(UInt8) -> Character` closure, not an encoding enum: EBCDIC and UTF-16 stay the caller's business. 9 tests. Bound by `AUIHexView` |
