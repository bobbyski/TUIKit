...
Phase 8 · Demo & Polish               ████████░░░░░░░░░░░░░░░░░░   30%  🔄 Demo refactored into per-window files; window state (8.9–8.12) done; Turbo theme (8.5–8.8) pending
...
### Turbo Pascal / Borland theme — 8.5–8.8, 8.13–8.15 ⏳ (planned)
...
| 8.6 | **Keyboard accelerator hints** | needed (main gap) | The highlighted mnemonic letter (Borland's red letters) on menus, buttons, labels, checkboxes — a `&`-marker or explicit index, drawn in an accent color, activated by Alt+letter. TUIKit has menu `keyEquivalent` but **no visual mnemonic and no per-control accelerator** — this is the primary feature to build. |
| 8.7 | **Border-embedded scrollbars** | core | The Borland/Turbo Pascal trick: draw a window's scrollbars *into its chrome* — the vertical bar riding the **right border** and the horizontal bar riding the **bottom border** — instead of reserving an interior column/row. This reclaims the column + row that `ScrollView`/`ListView`/etc. currently spend on the indicator, so the full text area stays usable. Needs: (a) a Panel/Window opt-in that hands the scroll indicator onto the border run (reusing the solid track/thumb styling, welding cleanly with the corner glyphs); (b) a **definable start offset** per bar so the run doesn't have to span the whole edge — most important on the **bottom** bar, which should begin where the text content begins (align with the gutter/content inset), not at the corner. Coordinate with `Divider`/junction drawing so an embedded bar and a border tee don't fight over the same cell. |
| 8.8 | Filled "pill" buttons + press animation | low | A filled-background button style (green pill), optionally with a drop shadow. Bobby: the **shadow itself is not important** — but *if* a button has a shadow, then on activate the label should "press down" onto the shadow position and pop back on release. The animation is conditional on the shadow: no shadow → no motion needed; shadow present → the press animation is wanted. |
| 8.13 | Themeable input-field well (background, not underline) | needed | Editable controls (`TextField`, and by extension `TextView` / `SyntaxTextView` / `ComboBox`) currently hardcode `.underline` to mark the editable area, so a theme can't swap it for a **contrasting background well** (Borland shows fields as e.g. black-on-cyan). Add a `field` slot (a `fieldForeground`/`fieldBackground` pair in the new model) — defaulting to underline so today's look is unchanged — and have the editable controls render their well with it. Then `.turbo` sets a blue field well with yellow text; `.standard` keeps the underline. Rolls into 8.14. |
...
| 8.15 | CSS is an on-top layer + demo toggle | needed | Clarify (in code + demo) that stylesheets (`StyleSheet`) apply **on top of** the resolved theme — theme (base → context) → CSS by specificity → drawn cell — and are **not** themes. Change the demo so CSS is an independent **toggle alongside** the theme dropdown (today it's presented as a `Theme ▸ CSS` entry, as if CSS were a theme): pick any theme, then flip the stylesheet on/off to see it layer over whatever's active. **Disabling CSS is just `styleSheet = nil`** — no separate state; the toggle is `view.styleSheet = on ? sheet : nil`, and the theme underneath is never touched. |
...
low-priority flourish; the red keyboard-hint letters (8.6) are the feature
