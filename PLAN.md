...
Phase 10 · VTG Vector Graphics        █████████████████████░░░░░   80%  🔄 Core shipped (10.1–10.5 first pass, 10.7; Ambiance theme); wider control pass under way — `ProgressIndicator` joined 2026-09-09 (a rounded track and a sub-cell gradient fill, with the cell bar shipping unchanged beside it per ground rule 8; 18 of 80 controls now emit chrome). Pending: real-VectorTerminal check (10.8) — **needs a person at a VectorTerminal, it cannot be closed headlessly** — and the rest of the control pass
...
Phase 20 · Frame Cost                 ███████████████████░░░░░░░   72%  🔄 v1 2026-09-09 — `present()` already diffed rows against the previous frame; compositing did not. `renderTree` now culls a subtree whose clip has no area (a `Painter` write and a `ChromeSurface` command both carry that clip, so nothing there could reach the buffer), the VTG owner id is built only where chrome is live, and the `TUIKIT_VTG_LOG` lookup happens once instead of per view per frame. A 200-row stack in a 60-row window: **17.3ms → 4.0ms a frame, 4.3×**. Remaining: `effectiveTheme` walks the whole superview chain and allocates per view per frame — the next candidate, and it needs a cache with real invalidation rather than a quick one; `TableView` gained a pull-based `RowSource` so a result set larger than the window costs the window (ActiveUI's table built ten thousand row views to draw twenty), and `TextField` gained `onBeginEditing`/`onEndEditing`, which is the hook validation needs and the control did not have
```
